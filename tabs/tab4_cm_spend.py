...
def render_tab4(expense_data, monthly_data, currency, current_month: str):
    ...
    # ── Top KPI strip ──
    # Show KPIs only when MONEY RECEIVED sheet is available (provides budget figures);
    # if it's missing, surface a clear info block instead.
    if mr is None:
        st.info(
            "The top-level budget KPIs and the Budget Flow chart are currently disabled "
            "because the sheet 'MONEY RECEIVED' is missing from the uploaded Expense file."
        )
    else:
        kpi_row([
        ...
        with col1:
            fig_flow = go.Figure(go.Waterfall(
                ...
                text=[fmt_currency(opening*mul, unit), fmt_currency(received*mul, unit),
                      fmt_currency((opening+received)*mul, unit),
                      f"-{fmt_currency(total_spent*mul, unit)}",
                      fmt_currency(balance*mul, unit)],
            ...
            st.plotly_chart(fig_flow, width='stretch', key=f"fig_flow_{current_month}")

        with col2:
            st.markdown(f"**Cumulative Spend Over {current_month}**")
            if not mr.empty and "Date" in mr.columns:
                ...
                st.plotly_chart(fig_cum, width='stretch', key=f"fig_cum_{current_month}")
            else:
    ...
    if ae is None:
        st.info(
            ...
            "'ACTIVITY EXP.' is missing from the uploaded Expense file."
        )
    else:
        ...
        other_total = oe["Amount_FCFA"].sum() if oe is not None and not oe.empty else 0
        ...
        for _, row in spend_by_type.iterrows():
            if row["Amount"] == 0:
                continue
            fig_stk.add_bar(name=row["SpendType"], x=[f"{current_month} 2026"], y=[row["Amount"]],
        ...
        st.plotly_chart(fig_stk, width='stretch', key=f"fig_stk_{current_month}")
    st.markdown("---")
    ...
    if oe is None:
        st.info(
            "This table is currently disabled because the sheet "
            "'OTHER EXP.' is missing from the uploaded Expense file."
        )
    elif not oe.empty:
        oe_display = oe.copy()
